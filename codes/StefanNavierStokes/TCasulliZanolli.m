...
function T = TCasulliZanolli(T)
    global rhoS hL Tc epsilonT
    % physical parameters of water and ice [in SI units]

    % dt = d/( max(lambdaS,lambdaL)/dx^2 + max(lambdaS,lambdaL)/dy^2 );         % time step restriction of FTCS
    ...
    tol = 1e-5*rhoS*hL;
    T0 = min(T0,Tc-epsilonT); % Initial guess for the outer iterations, see the paper by Casulli & Zanolli
    MaxNewton = 100;
    % ----- OUTER iterations -------
    for iouter = 1:MaxNewton
        ...
        % ----- INNER iterations ------
        T0 = max(T0,Tc-epsilonT);   % Initial guess for the inner iterations, see the paper by Casulli & Zanolli
