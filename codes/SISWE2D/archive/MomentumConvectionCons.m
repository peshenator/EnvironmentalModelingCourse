...
function [u,v] = MomentumConvectionCons(Hub,Hvb,Hb,u,v)
global Nx Ny dt dx dy;

F = zeros(Nx+1,Ny,2);
G = zeros(Nx,Ny+1,2);

F(2:Nx,:,1) = 0.5*( u(2:Nx,:) .*( Hub(2:Nx,:) + Hub(1:Nx-1,:) ) ...
              - abs(u(2:Nx,:)).*( Hub(2:Nx,:) - Hub(1:Nx-1,:) ) );
F(2:Nx,:,2) = 0.5*( u(2:Nx,:) .*( Hvb(2:Nx,:) + Hvb(1:Nx-1,:) ) ...
              - abs(u(2:Nx,:)).*( Hvb(2:Nx,:) - Hvb(1:Nx-1,:) ) );

G(:,2:Nx,1) = 0.5*( v(:,2:Nx) .*( Hub(:,2:Nx) + Hub(:,1:Nx-1) ) ...
              - abs(v(:,2:Nx)).*( Hub(:,2:Nx) - Hub(:,1:Nx-1) ) );
G(:,2:Nx,2) = 0.5*( v(:,2:Nx) .*( Hvb(:,2:Nx) + Hvb(:,1:Nx-1) ) ...
              - abs(v(:,2:Nx)).*( Hvb(:,2:Nx) - Hvb(:,1:Nx-1) ) );
